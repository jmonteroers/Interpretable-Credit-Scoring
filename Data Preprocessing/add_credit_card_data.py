...
df = add_count(
    df, cc_balance, PREV_ID, "COUNT_ACTIVE_CC_CURR", (cc_balance.MONTHS_BALANCE == -1) & (cc_balance.AMT_BALANCE > EPS)
    )

# Feature - Over the limit, current
df = add_count(
    df, cc_balance, PREV_ID, "COUNT_CC_OVER_LIMIT_CURR", (cc_balance.MONTHS_BALANCE == -1) & (cc_balance.AMT_BALANCE > cc_balance.AMT_CREDIT_LIMIT_ACTUAL)
)
